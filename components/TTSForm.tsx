...
import { VOICES, EXPRESSIONS, PLAN_LIMITS } from '../constants';
import { VoiceName, TTSSettings, CustomVoiceProfile, Plan } from '../types';
import { generateTTS } from '../services/geminiTTS';

interface TTSFormProps {
  onGenerate: (text: string, voice: VoiceName, settings: TTSSettings) => Promise<void>;
  isLoading: boolean;
  prefill?: { voice: VoiceName, settings: TTSSettings, profileId?: string };
  customVoices: CustomVoiceProfile[];
  charsUsed: number;
  subscription: Plan;
}

export const TTSForm: React.FC<TTSFormProps> = ({ onGenerate, isLoading, prefill, customVoices, charsUsed, subscription }) => {
  ...
  const [activeMenu, setActiveMenu] = useState<'voice' | 'settings' | 'expression' | 'enhancement' | null>(null);
  ...
  const estimatedSeconds = Math.round((words / 150) * 60 / settings.speed);
  const readingTimeText = estimatedSeconds < 60 ? `${estimatedSeconds}s` : `${Math.floor(estimatedSeconds/60)}m ${estimatedSeconds%60}s`;

  useEffect(() => {
    if (prefill) {
      setVoice(prefill.voice);
      setSettings(prefill.settings);
      setActiveProfileId(prefill.profileId || null);
  ...
  const toggleMenu = (menu: 'voice' | 'settings' | 'expression' | 'enhancement') => {
  ...
  const handleCustomVoiceSelect = (v: CustomVoiceProfile) => {
    setVoice(v.baseVoice);
  ...
  const selectedCustom = activeProfileId ? customVoices.find(v => v.id === activeProfileId) : null;
    ...
    <div className="w-full flex flex-col gap-6" ref={formRef}>
        ...
        <div className="w-full bg-gradient-to-r from-blue-900/40 to-indigo-900/40 border border-blue-500/30 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div className="text-center md:text-left">
              <h4 className="text-xs md:text-sm font-black text-white uppercase tracking-wider">Upgrade to Premium or VVIP for Professional Tier.</h4>
              <p className="text-[10px] text-blue-300 font-bold uppercase tracking-widest mt-1">Connect on WhatsApp: +260765546444</p>
            </div>
          </div>
          <a href="https://wa.me/260765546444" target="_blank" className="px-5 py-2 bg-white text-blue-900 text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-blue-50 transition-all whitespace-nowrap">Go Pro</a>
        </div>
      )}
      ...
      <div className={`bg-[#0a0f1d]/60 backdrop-blur-xl border rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 shadow-2xl relative transition-all ${isLimitReached ? 'border-red-500/50 ring-2 ring-red-500/10' : 'border-white/10 focus-within:ring-2 focus-within:ring-blue-500/20'}`}>
        ...
        <div className="mt-4 flex flex-wrap justify-between items-center gap-4 px-2">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-600">
                Est. Duration: <span className="text-blue-500">{readingTimeText}</span>
              </span>
            </div>
          </div>
          <span className="hidden md:block text-[10px] font-bold text-slate-700 uppercase tracking-widest">Resets at midnight CAT</span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-6 w-full px-2">
        <div className="flex flex-wrap justify-center items-center gap-3 w-full">
          ...
          {/* Voice Menu */}
          <div className="relative">
            <label className="block text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Select Voice</label>
            <button 
              type="button"
                ...
                <div className="space-y-4 max-h-[25rem] overflow-y-auto custom-scrollbar">
                  {customVoices.length > 0 && (
                    <div className="space-y-1">
                      <h4 className="text-[8px] font-black text-blue-500 uppercase tracking-widest px-2 mb-2">Saved Personas</h4>
                      {customVoices.map((v) => (
                        <button
                          key={v.id}
                          onClick={() => handleCustomVoiceSelect(v)}
                          className={`w-full text-left px-3 py-2.5 rounded-xl transition-all flex items-center justify-between ${
                            activeProfileId === v.id ? 'bg-blue-600 text-white' : 'hover:bg-white/5 text-slate-400'
                          }`}
                        >
                          <div className="flex flex-col">
                          ...
                          {activeProfileId === v.id && <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                        </button>
                      ))}
                    </div>
                  )}
                  <div className="space-y-1">
                    <h4 className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-2 mb-2">Signature Voices</h4>
                    {VOICES.map((v) => (
                      <div
                        key={v.id}
                        className={`group/voiceitem w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer ${
                          (voice === v.baseVoice && !activeProfileId) ? 'bg-blue-600 text-white' : 'hover:bg-white/5 text-slate-400'
                        }`}
                      ...
                      >
                        ...
                        >
                           {isPreviewing === v.id ? <div className="w-3 h-3 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" /> : <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}
                        </button>
          ...
          {/* Delivery Style */}
          <div className="relative">
            <label className="block text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Vocal Mood</label>
            <button 
              type="button"
                ...
                <div className="space-y-1">
                  {EXPRESSIONS.map((e) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest ${
          ...
          {/* Enhancement Presets */}
          <div className="relative">
            <label className="block text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Enhancement</label>
            <button 
              type="button"
              onClick={() => toggleMenu('enhancement')}
              className={`flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg transition-all border ${
                activeMenu === 'enhancement' ? 'bg-blue-600 text-white border-blue-400' : 'bg-[#0a0f1d] text-slate-300 border-white/10 hover:border-white/30'
              }`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-7.714 2.143L11 21l-2.286-6.857L1 12l7.714-2.143L11 3z" strokeWidth="2.5"/></svg>
              <span className="text-[10px] md:text-xs font-black uppercase tracking-widest">{settings.enhancement || 'None'}</span>
            </button>
            {activeMenu === 'enhancement' && (
              <div className="absolute bottom-full mb-4 left-0 w-52 bg-[#0a0f1d] border border-white/10 rounded-2xl shadow-2xl p-3 z-50 animate-in fade-in zoom-in-95 duration-200">
                <div className="space-y-1">
                  {['None', 'Studio', 'Warmth', 'Compressed'].map((e) => (
                    <button
                      key={e}
                      onClick={() => { 
                        setSettings({...settings, enhancement: e.toLowerCase() as any}); 
                        setActiveMenu(null); 
                      }}
                      className={`w-full text-left px-4 py-2.5 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest ${
                        (settings.enhancement || 'none') === e.toLowerCase() ? 'bg-blue-600 text-white' : 'hover:bg-white/5 text-slate-400'
                      }`}
                    >
                      {e}
                    </button>
          ...
          <div className="relative">
            <label className="block text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Controls</label>
            <button 
              type="button"
        ...
        >
          {isLoading ? (
            ...
            <>
              <span className="text-lg md:text-xl uppercase tracking-widest">{isLimitReached ? 'Limit Reached' : 'Generate Audio'}</span>
