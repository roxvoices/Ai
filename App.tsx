...
const App: React.FC = () => {
  ...
  const refreshUserData = async (userId: string, email: string, isInitialLogin = false) => {
    try {
      ...
      console.error("Refresh user data failure:", err);
  ...
  useEffect(() => {
    ...
    // Hard fail-safe: Force stop loading after 5 seconds
  ...
  const handleRegister = async (name: string, email: string, pass: string) => {
    ...
    // Attempt profile creation immediately
    const isAdmin = ADMIN_EMAILS.includes(email.toLowerCase());
    try {
      ...
      console.warn("Initial profile upsert failed, likely waiting for email confirmation.");
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#020617]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Authenticating...</p>
        </div>
      </div>
    );
  }
